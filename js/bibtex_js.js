...
function BibtexDisplay() {
  ...
  this.translateAuthor = function(string){
      var arrayString = string.split("=");
      var newString = arrayString[0];
      if(arrayString.length > 1){
          newString="<a href=\""+arrayString[1].replace(/[\[\]\"\s\t\n]+/g,"")+
              "\">"+newString+"<\a>";
      }
      return newString;
  }
  ...
  this.displayAuthor = function(string){
  	...
  	var arrayString = string.split(new RegExp("[\\s]+and[\\s]+"));
  	  var newString = this.translateAuthor(arrayString[0]);
  	for (i = 1; i < arrayString.length; i++) {
  	  if(i+1>=arrayString.length) {
  	      newString += ", and " + this.translateAuthor(arrayString[i]);
  	  } else {
	      newString += ", " + this.translateAuthor(arrayString[i]);
	  }
	}
	return newString;
  }
    ...
    for (var index in keys) {
      ...
      if(key=="BIBTEXRAW") {
        tpl.find("." + key.toLowerCase()).html(value);
          ...
          var temp = this.fixValue(value);
          temp = this.displayAuthor(value);
      	  tpl.find("span:not(a)." + key.toLowerCase()).html("<span>"+temp+"</span>");

          
      } else if(key == "URL"){
          var temp1 = tpl.find("span.title_pdf");
          var temp2 = temp1.children("a");
          temp2.attr("href",this.fixValue(value));
      }else {
        //tpl.find("span:not(a)." + key.toLowerCase()).css("color","red");
